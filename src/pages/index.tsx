...
export default function Home() {
  ...
  const handleDragEnd = (result) => {
    if (!result.destination || !todos) return; // Ensure todos exist
  
    const reorderedTodos = [...todos]; // Create a copy of todos
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-gray-900 to-gray-800 text-white flex flex-col items-center py-12 px-4">
      <div className="flex justify-between w-full max-w-lg">
        <h1 className="text-4xl font-extrabold mb-6 text-blue-400 shadow-md">T3 To-Do List</h1>
        <button onClick={handleLogout} className="bg-red-500 text-white px-4 py-1 text-sm rounded-md hover:bg-red-600 transition duration-200">Logout</button>
      </div>
